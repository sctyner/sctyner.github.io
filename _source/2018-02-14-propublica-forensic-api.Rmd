...
```{r mapp, echo=TRUE, fig.align='center', fig.height=6, cache = TRUE}
# mapping the offices 
...
```{r purrrdat, cache=TRUE, fig.align='center', fig.width=10, fig.height=6}
...
```{r dropnas, echo = FALSE}
alldat4 <- alldat3 %>% select(-c(geo.district, geo.note, geo.population, district, note, population))
```
...
```{r paths, fig.align='center', fig.width=10, fig.height=6}
paths <- alldat4 %>% 
...
```{r ucpaths, fig.align='center', cache=TRUE, fig.width=10}
...
```{r fig.align='center',  fig.height=6, cache = TRUE}
# separate states and counties
cnty_data <- alldat4 %>% 
  filter(geo.kind !="state") %>% 
...
# map
ggplot() + 
  geom_sf(data = cnty_geos_map2, aes(fill = autopsy_ratio, geometry = geometry), color = "white") + 
  #geom_sf(data = state_geos_map2, aes(fill = autopsy_ratio, geometry = geometry)) +
  #scale_fill_manual(values = c("#e41a1c","#377eb8","#4daf4a","#984ea3","white")) + 
  coord_sf(crs = st_crs(state_geos_map$geometry)) + 
  theme_void() + 
  theme(panel.grid.major = element_line(color = "white")) + 
